...
proc murmur32(str:string, seed:uint(32)=0) {
   ...
   var tail = (blocks[(nblocks-1)*4..(nblocks-1)*4+3]);
   var k1 : uint(32) = 0;
...
record HyperLogLog {
  ...
  /*
  proc estimate_bias(E, p) {
    var bias_vector = biasData(p-4);
    var nearest_neighbors = get_nearest_neighbors(E, rawEstimateData[p-4]);
    return (+ reduce [ i in nearest_neighbors ] bias_vector(i))/nearest_neighbors.domain.size();
  }

  proc card(val:string) {
    var V = + reduce [ i in this.M.domM ] 1;

    if V > 0 {
      var H = this.m * log(m:real/V:real);
      return if H = get_threshold(this.p) then H else _Ep();
    }
